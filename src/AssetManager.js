...
class AssetManager {
  ...
  _loadAudios() {
    const { audios, listener, audioLoader } = this;
    ...
    audioLoader.load(require("./audio/step1.ogg"), (buffer) =>
      step1.setBuffer(buffer)
    );
    audioLoader.load(require("./audio/step2.ogg"), (buffer) =>
      step2.setBuffer(buffer)
    );
    audioLoader.load(require("./audio/shot.ogg"), (buffer) =>
      shot.setBuffer(buffer)
    );
    audioLoader.load(require("./audio/reload.ogg"), (buffer) =>
      reload.setBuffer(buffer)
    );
    audioLoader.load(require("./audio/impact1.ogg"), (buffer) =>
      impact1.setBuffer(buffer)
    );
    audioLoader.load(require("./audio/impact2.ogg"), (buffer) =>
      impact2.setBuffer(buffer)
    );
    audioLoader.load(require("./audio/impact3.ogg"), (buffer) =>
      impact3.setBuffer(buffer)
    );
    audioLoader.load(require("./audio/impact4.ogg"), (buffer) =>
      impact4.setBuffer(buffer)
    );
    audioLoader.load(require("./audio/impact5.ogg"), (buffer) =>
      impact5.setBuffer(buffer)
    );
    audioLoader.load(require("./audio/empty.ogg"), (buffer) =>
  ...
  _loadModels() {
    ...
    gltfLoader.load(require("./model/target.glb"), (gltf) => {
    ...
    gltfLoader.load(require("./model/gun.glb"), (gltf) => {
      ...
      const texture = textureLoader.load(require("./model/muzzle.png"));
    ...
    const texture = textureLoader.load(require("./model/bulletHole.png"));
    texture.minFilter = THREE.LinearFilter;
    const bulletHoleGeometry = new THREE.PlaneBufferGeometry(0.1, 0.1);
    const bulletHoleMaterial = new THREE.MeshLambertMaterial({
      map: texture,
      transparent: true,
      depthWrite: false,
      polygonOffset: true,
    ...
    bulletLineGeometry.setFromPoints([
      new THREE.Vector3(),
      new THREE.Vector3(0, 0, -1),
    ]);

    const bulletLine = new THREE.LineSegments(
      bulletLineGeometry,
      bulletLineMaterial
    );
  ...
  _loadAnimations() {
    ...
    positionKeyframes = new THREE.VectorKeyframeTrack(
      ".position",
      [0, 0.05, 0.15, 0.3],
      [0.3, -0.3, -1, 0.3, -0.2, -0.7, 0.3, -0.305, -1, 0.3, -0.3, -1]
    );

    q0 = new THREE.Quaternion();
    q1 = new THREE.Quaternion().setFromAxisAngle(
      new THREE.Vector3(1, 0, 0),
      0.2
    );
    q2 = new THREE.Quaternion().setFromAxisAngle(
      new THREE.Vector3(1, 0, 0),
      -0.02
    );

    rotationKeyframes = new THREE.QuaternionKeyframeTrack(
      ".rotation",
      [0, 0.05, 0.15, 0.3],
      [
        q0.x,
    ...
    const shotClip = new THREE.AnimationClip("Shot", 0.3, [
      positionKeyframes,
      rotationKeyframes,
    ]);
    ...
    positionKeyframes = new THREE.VectorKeyframeTrack(
      ".position",
      [0, 0.2, 1.3, 1.5],
      [0.3, -0.3, -1, 0.3, -0.6, -1, 0.3, -0.6, -1, 0.3, -0.3, -1]
    );
    ...
    const reloadClip = new THREE.AnimationClip("Reload", 1.5, [
      positionKeyframes,
      rotationKeyframes,
    ]);
